...
function submitDataAndExportToCSV() {
	
	var eventTitle        = $("#eventTitle").val();
	var isAllDayEvent     = $("#allDayEvent").prop("checked");
	var eventHeStartDay   = $('#heStartDay').find(":selected").val();
	var eventHeStartMonth = $('#heStartMonth').find(":selected").val();
	var eventHeStartHour  = $('#heStartHour').find(":selected").text();
	var eventHeEndDay     = $('#heEndDay').find(":selected").val();
	var eventHeEndMonth   = $('#heEndMonth').find(":selected").val();
	var eventHeEndHour    = $('#heEndHour').find(":selected").text();
	var eventYearlyRepeat = $('#yearlyRepeat').find(":selected").val();
	
	console.log("title: " + eventTitle + "; isAllDayEvent: " + isAllDayEvent + 
				"; Start Day: " + eventHeStartDay + "; Start Month: " + eventHeStartMonth + 
				"; Start Hour: " + eventHeStartHour  + "; Yearly Repeat: " + eventYearlyRepeat + 
				"; End Day: " + eventHeEndDay + "; End Month: " + eventHeEndMonth + "; End hour: " + eventHeEndHour);
	
	if (eventTitle == "") {	
		alert("Please fill in Event Title");
	} else {
		var titlesAndHeDates = buildJSONArrayString(eventTitle, isAllDayEvent, eventHeStartDay, eventHeStartMonth, eventHeStartHour, eventHeEndDay, eventHeEndMonth, eventHeEndHour, eventYearlyRepeat);
		exportToGCalCsv(titlesAndHeDates);
	}
	
}

function buildJSONArrayString(eventTitle, isAllDayEvent, eventHeStartDay, eventHeStartMonth, eventHeStartHour, eventHeEndDay, eventHeEndMonth, eventHeEndHour, eventYearlyRepeat) {
	
	var JSONArrayString = "[{'title': '" + eventTitle + 
						  "' ,'isAllDayEvent': " + isAllDayEvent + 
						  ", 'heDayStart':" + eventHeStartDay + 
						  " ,'heMonthStart': " + eventHeStartMonth + 
						  " ,'heHourStart': '" + eventHeStartHour + 
						  "', 'heDayEnd': " + eventHeEndDay + 
						  ", 'heMonthEnd': " + eventHeEndMonth + 
						  ", 'heHourEnd': '" + eventHeEndHour + 
						  "', 'eventYearlyRepeat': " + eventYearlyRepeat + 
						  "}]";
...
function exportToGCalCsv(titlesAndHeDates) {
		
	var titlesAndHeDatesObject = eval('(' + titlesAndHeDates + ')');
	...
	for (var i = first_year; i <= last_year; i++) {
		for (var j in titlesAndHeDatesObject) {
		   var iYearHeDateStart = new HeDate(i, 
			...
			var iYearHeDateEnd = new HeDate(i, 
										titlesAndHeDatesObject[j]["heMonthEnd"], 
										titlesAndHeDatesObject[j]["heDayEnd"]);
			data.push({
				title: titlesAndHeDatesObject[j]["title"],
				isAllDayEvent: titlesAndHeDatesObject[j]["isAllDayEvent"],
				startDate: iYearHeDateStart.ConvertToGregorian().toString(),
				startTime: titlesAndHeDatesObject[j]["heHourStart"],
				endDate: iYearHeDateEnd.ConvertToGregorian().toString(),
				endTime: titlesAndHeDatesObject[j]["heHourEnd"]
			});
		}
...
function exportDataToCSV(unformattedData) {
	
	  var headers = {
		  title: 'Subject'.replace(/,/g, ''), // remove commas to avoid errors
	  ...
	  unformattedData.forEach((item) => {
		  ...
		  var isAllDayEventString;
		  
		  if (item.isAllDayEvent) {
			isAllDayEventString = "TRUE";
		  } else {
			isAllDayEventString = "FALSE";
		  }
		  ...
		  formattedData.push({
			  ...
			  allDayEvent: isAllDayEventString
		  });
	  });
	  
	 var fileTitle = 'importToGCal';
...
function calculateLastYearFromYearlyRepeat(eventYearlyRepeat) {
	
	var currentHebrewYear = getCurrentHebrewYear();
	
	switch (eventYearlyRepeat) {
		
		case 1: return currentHebrewYear;
		case 2: return currentHebrewYear + 5;
		case 3: return currentHebrewYear + 10;
		case 4: return currentHebrewYear + 50;
		case 5: return currentHebrewYear + 100;
		default: return currentHebrewYear;
...
$(document).ready(function() {
	...
	// All day event & Hours dependency
	$('#allDayEvent').change(function() {
        if(this.checked) {
            $(this).prop("checked", true);
			$("#heStartHour").prop("disabled", true);
			$("#heEndHour").prop("disabled", true);
        } else {
			$(this).prop("checked", false);
			$("#heStartHour").prop("disabled", false);
			$("#heEndHour").prop("disabled", false);
		}
    });
	...
	$('#add_event').click(function() {
		var e = $('<select class="selectpicker" id="yearlyRepeat"><option value="1">בלי חזרות - השנה הנוכחית בלבד</option><option value="2">חזור למשך 5 השנים הקרובות</option><option value="3">חזור למשך 10 השנים הקרובות</option><option value="4">חזור למשך 50 השנים הקרובות</option><option value="5">חזור למשך 100 השנים הקרובות</option></select>');
		$('#container').append(e);    
